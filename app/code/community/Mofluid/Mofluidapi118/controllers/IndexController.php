...
class Mofluid_Mofluidapi118_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
        try {
            ...
            $ddate                    = $_GET["ddate"];
            $dtime                    = $_GET["dtime"];
            $loyaltyInstruction       = $_GET["loyaltyInstruction"];
            $loyaltyInfo              = $_GET["loyaltyInfo"];
            $filterbrand[]            = $_GET["filterbrand"];
            $filtercategory[]         = $_GET["filtercategory"];
            $lastUpdatedAt            = $_GET["lastUpdatedAt"];
            ...
            if ($store == -1 || $store == null || $store == "") {
                ...
                if ($service == "sidecategory") {
                    ...
                    $res = $ws_service->ws_products_more($store, $service, $categoryid, $curr_page, $page_size, $filterbrand, $filtercategory, $sortType, $sortOrder, $currency);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                    ...
                    $res = $ws_service->ws_search($store, $service, $search_data, $curr_page, $page_size, $filterbrand, $filtercategory, $sortType, $sortOrder, $currency);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                    ...
                    $res = $ws_service->placeorder($custid, $products, $store, $address, $couponCode, $is_create_quote, $transid, $pmethod, $smethod, $currency, $messages, $theme,$ddate,$dtime,$loyaltyInstruction, $loyaltyInfo);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                ...
                else if ($service == "slottiming") {
                    $res = $ws_service->ws_slottiming($store, $service);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "orderdeliverydate") {
                    $res = $ws_service->ws_orderdeliverydate();
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "setorderdelivery") {
                    $res = $ws_service->ws_set_order_delivery();
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "colorswatchpro") {
                    $res = $ws_service->color_swatch_pro($store, $service);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "getminimumordervalue") {
                    $res = $ws_service->get_minimum_order_value($store, $service);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "savecart") {
                    $res = $ws_service->addtoQuote($custid, $products, $store);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                }
                else if ($service == "synccart") {
                    $res = $ws_service->syncCart($custid, $products, $store, $lastUpdatedAt);
                    echo $_GET["callback"] . '(' . json_encode($res) . ');';
                } 
