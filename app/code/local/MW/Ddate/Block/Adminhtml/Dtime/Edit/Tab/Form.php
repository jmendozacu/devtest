...
class MW_Ddate_Block_Adminhtml_Dtime_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  /*protected function _prepareForm()
  {
		...
		$this->setTemplate('ddate/dtimetable/edit.phtml');
	    ...
	    } elseif ( Mage::registry('dtimetable_data') ) {
	          $form->setValues(Mage::registry('dtimetable_data')->getData());
	    }
  ...
  protected function _prepareForm()
  {
      $form = new Varien_Data_Form();
      $this->setForm($form);
      $fieldset = $form->addFieldset('general_form', array('legend'=>Mage::helper('ddate')->__('General information')));
      ...
      if ( Mage::getSingleton('adminhtml/session')->getDtimeData() )
      {
          ...
          if (!Mage::app()->isSingleStoreMode()){
				...
				$collection->getSelect()->join(array('mwdtime_store'=>$collection->getTable('dtime_store')), 'mwdtime_store.dtime_id = main_table.dtime_id AND main_table.dtime_id='.$catc->getDtimeId(), array('mwdtime_store.store_id'));
