...
class Service
{
    ...
    public function ws_products($store_id, $service, $categoryid, $curr_page, $page_size, $sortType, $sortOrder, $currentcurrencycode)
    {
        ...
        ))->addAttributeToFilter('visibility',2)->addAttributeToFilter('is_in_stock', array(
            'in' => array(
                $is_in_stock_option,
                1
            )
    ...
    public function ws_getNewProducts($currentcurrencycode, $service, $store)
    {
        ...
        $_products = Mage::getModel('catalog/product')->getCollection()->joinField('is_in_stock', 'cataloginventory/stock_item', 'is_in_stock', 'product_id=entity_id', '{{table}}.stock_id=1', 'left')->addStoreFilter($store)->setOrder('created_at', 'desc')->addAttributeToFilter('visibility',2);
        $_products->addAttributeToSelect('*');
