...
Product.Config.prototype.fillSelect = function(element){
    ...
    if(options) {
        ...
        for(var i=0;i<options.length;i++){
		    ...
		    if (options[i].isSoldOut) {		
			image.addClassName('amconf-sold-out');
		    }
                    image.addClassName('amconf-image');
		    image.alt = options[i].label;
		    image.title = options[i].label;		    
		    var t = document.createTextNode(options[i].label);
		    image.appendChild(t);
		    image.setAttribute("href", "javascript:void(0);");
			image.setAttribute("data-id", options[i].pid);
            image.setAttribute("data-qty", options[i].cartQty);
