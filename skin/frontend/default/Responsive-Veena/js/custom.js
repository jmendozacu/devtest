...
// 	});
        ...
        $k('.cart-label').live("click",function() {
   		 $k('#panel').slideToggle();
		 $k("#panel").parent().toggleClass('active').parent().find().slideToggle();
 	});
	...
	$k(".tm_headerlinks_inner").click(function(){
		$k(".tm_headerlinkmenu .mobile_link_menu").toggle('slow');
	});
	
	$k(".links a").hover(function() {
	$k(this).next("em").animate({opacity: "show", top: "-24px"}, "slow");
	}, function() {
...
var $n =jQuery.noConflict();
	$n( document ).ready(function() {
 	...
 	$n('.amconf-image-container a').each(function(){
	   	obj = this;
	   	changeClass(obj);
	   	var c=$n('#'+obj.id+' a').attr('id');
	   	
 	});
 	 	$n('.qty-indicater').each(function(){
	   	obj = this;
	   	$n(this).parents('.product-block').addClass("selected-shadow");
   ...
   function changeClass(obj){
   	var qty = 0;
	   	var qty  = $n('#'+obj.id).attr( "data-qty");
	
	    if(qty>0){
	    	
	    
	    $n('<span class="qty-indicater">'+qty+'</span>').insertAfter('#'+obj.id);
		...
		$n(window).bind("load", function() { 
 		...
 		count = 0;
 		var content = $n('#top_link1 a').text();
 		if(content.search("Login")!=-1){
 			var buffer = 0;
 		}
 		else {
 			var buffer = 40;
 		}
 	$n('#grid li .product-block-inner').each(function(){
	   	obj = this;
	   	if(count%4==0) {
		   			   		   	
		   height = $n(this).find(' #amconf-block').height();
		  two = $n(this).parents('li').next().next('li').find('.product-block-inner #amconf-block').height();
		   	if(height<two) {
		   		height = two;
		   	}
		   three = $n(this).parents('li').next().next('li').next().next('li').find('.product-block-inner #amconf-block').height();
		   	if(height<three) {
		   		height = three;
		   	}
		   	four = $n(this).parents('li').next().next('li').next().next('li').next().next('li').find('.product-block-inner #amconf-block').height();
		   	if(height<four) {
		   		height = four;
		   	}
		   	nameHeight = $n(this).find('.product-name').height();
		  	nametwo = $n(this).parents('li').next().next('li').find('.product-block-inner .product-name').height();
		   	if(nameHeight<nametwo) {
		   		nameHeight = nametwo;
		   	}
		   namethree = $n(this).parents('li').next().next('li').next().next('li').find('.product-block-inner .product-name').height();
		   	if(nameHeight<namethree) {
		   		nameHeight = namethree;
		   	}
		   	namefour = $n(this).parents('li').next().next('li').next().next('li').next().next('li').find('.product-block-inner .product-name').height();
		   	if(nameHeight<namefour) {
		   		nameHeight = namefour;
		   	}

		   	$n(this).parents('li ').find('.product-block-inner #amconf-block').height(height);
		   	$n(this).parents('li').next().next('li').find('.product-block-inner #amconf-block').height(height);
		   	$n(this).parents('li').next().next('li').next().next('li').find('.product-block-inner #amconf-block').height(height);
		   	$n(this).parents('li').next().next('li').next().next('li').next().next('li').find('.product-block-inner #amconf-block').height(height);
			
			$n(this).parents('li ').find('.product-block-inner .product-name').height(nameHeight);
		   	$n(this).parents('li').next().next('li').find('.product-block-inner .product-name').height(nameHeight);
		   	$n(this).parents('li').next().next('li').next().next('li').find('.product-block-inner .product-name').height(nameHeight);
		   	$n(this).parents('li').next().next('li').next().next('li').next().next('li').find('.product-block-inner .product-name').height(nameHeight);

		   	totalHeight = height +nameHeight +280+buffer;
		   	$n(this).parents('li ').height(totalHeight);
		   	$n(this).parents('li').next().next('li').height(totalHeight);
		   	$n(this).parents('li').next().next('li').next().next('li').height(totalHeight);
		   	$n(this).parents('li').next().next('li').next().next('li').next().next('li').height(totalHeight);
