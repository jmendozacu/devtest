...
class Mage_Catalog_Block_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Abstract
{
  ...
  foreach ($this->getAllowAttributes() as $attribute) {
            ...
            if (is_array($prices)) {
                foreach ($prices as $value) {
                 ...
                 $info['options'][] = array(
                        ...
                        'label'         => ($options['qty'][$value['label']] <= 0) ? $value['label'] . ' (SOLD OUT)' : $value['label'],
                        'isSoldOut'     => ($options['qty'][$value['label']] <= 0) ? true : false,
        ...
        $config = array(
            ...
            'chooseText'        => Mage::helper('catalog')->__('Select Pack Size'),
            'taxConfig'         => $taxConfig
        );
