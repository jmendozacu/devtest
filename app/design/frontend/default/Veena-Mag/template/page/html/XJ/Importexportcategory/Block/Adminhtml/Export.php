 ...
 * @author     XJ 
 */
class XJ_Importexportcategory_Block_Adminhtml_Export extends Mage_Adminhtml_Block_Abstract
{
	...
	public function saveCsvContent($data, $model) {
		...
		foreach ($data as $head => $value) {
			...
			if ($value['function'] == "getCatproid") {
				$str2 = $model->getId();
				$str1 = $this->$value['function']($str2);
			}
	...
	public function getCatproid($val) {
		$array = array();
		$category = Mage::getModel('catalog/category')->load($val);
	...
	public function getScope($val) {
		$retarray = Mage::getModel('elsner_core/attribute')->getScopes();
		if (isset($retarray[$val])) return $retarray[$val];
		return $val;
	}

	public function getLayeredNavigation($val) {
		$retarray = Mage::getModel('elsner_core/attribute')->getLayeredNavigations();
		if (isset($retarray[$val])) return $retarray[$val];
		return $val;
	}

	public function getInputType($val) {
		$retarray = Mage::getModel('elsner_core/attribute')->getInputTypes();
		if (isset($retarray[$val])) return $retarray[$val];
		return $val;
	}

	public function getInputValidation($val) {
		$retarray = Mage::getModel('elsner_core/attribute')->getInputValidations();
		if (isset($retarray[$val])) return $retarray[$val];
		return $val;
	}
