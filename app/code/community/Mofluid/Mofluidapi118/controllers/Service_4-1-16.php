...
class Service
{
    ...
    public function ws_slottiming($store, $service, $custid){
   ...
   public function ws_search($store, $service, $search_data, $curr_page, $page_size, $sortType, $sortOrder, $currentcurrencycode)
    {
        ...
        try {
            ...
            $total_product_collection = Mage::getResourceModel('catalog/product_collection')->joinField('is_in_stock', 'cataloginventory/stock_item', 'is_in_stock', 'product_id=entity_id', '{{table}}.stock_id=1', 'left')->addStoreFilter($store)->addAttributeToSelect('*')->addAttributeToFilter('name', $search_condition)->addStoreFilter($store)->setVisibility(array(3,4))->addAttributeToFilter('type_id', array(
                'in' => array(
                    'configurable',
                    'grouped',
                    'simple'
...
if ($_product->getTypeID() == 'grouped') {
   ...
   // placeorder($custid, $products, $store, $address, $couponCode, $is_create_quote, $transid, $pmethod, $smethod, $currency, $messages, $theme,$ddate,$dtime);
    public function placeorder($custid, $Jproduct, $store, $address, $couponCode, $is_create_quote, $transid, $payment_code, $shipping_code, $currency, $message,$theme,$ddate,$dtime)
    {
