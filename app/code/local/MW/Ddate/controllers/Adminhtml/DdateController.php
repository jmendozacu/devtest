...
class MW_Ddate_Adminhtml_DdateController extends Mage_Adminhtml_Controller_action
{
 	...
 	public function gridAction()
    {
        $history = $this->getLayout()
            ->createBlock('ddate/adminhtml_ddate_edit_tab_order', 'ddate.grid.order');
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($ddateIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
	...
	public function massResetAction() {
        ...
        if(!is_array($ddateIds)) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully cleared', count($ddateIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
	...
	function isenableddayAction() {	
		$data=$this->getRequest()->getPost();
		$slotId = (isset($data['mwtime'])) ? $data['mwtime']: '';
		$date = (isset($data['mwdate'])) ? $data['mwdate'] : '';
		if(empty($date) || empty($slotId)) { echo $this->build_return_message('empty',Mage::helper('ddate')->__('Delivery Date or Time is empty!'));return; }
		if($slotId=='deleted') { echo $this->build_return_message('deleted',Mage::helper('ddate')->__('This time slot had been deleted.Please choose an other time slot!'));return; }
		...
		$data['orderid']= (string) $data['orderid'];
	 	if(!empty($data['orderid'])){		           
			...
			if ($ddates->count() == 1):               
					...
					$ddates_store=Mage::getModel('ddate/ddate_store')->getCollection()->addFieldToFilter('increment_id', $data['orderid']);					
					...
					if($ddates_store->count()== 1){	
						...
						if($old_ddate->getOrdered()== 1) $old_ddate->delete();
						else {
				...
				$ddates_store = Mage::getModel('ddate/ddate_store')->getCollection()->addFieldToFilter('increment_id', $data['orderid']);
					...
					$ddates_store=$ddates_store->getFirstItem();
					$old_ddate=Mage::getModel('ddate/ddate')->load($ddates_store->getDdateId());					
					if($old_ddate->getOrdered()== 1)$old_ddate->delete();else {
					$old_ddate->setOrdered($old_ddate->getOrdered() - 1)->save();
					$ddates_store->delete();
					}
			...
			echo $this->build_return_message('ok',Mage::helper('ddate')->__('Updated successfully!'));
		}else echo $this->build_return_message('error',Mage::helper('ddate')->__('Update failed!')); 
		return ;
	} 
	 /**
    ...
    function isEnabled($slotId, $date) {
        ...
        if ($slot->getHoliday() == 1 && Mage::helper('ddate')->getDayoff()) {
            {  return $this->ajaxerror=Mage::helper('ddate')->__('This is Holiday or Dayoff');}
        }         
		/* check ordered items */
		$ordered=Mage::helper('ddate')->ordered_counting($date,$slotId);	
		if($ordered && $ordered >= $slot->getMaximumBooking()){
		  return $this->ajaxerror=Mage::helper('ddate')->__('This slot time has been full booking');		  
		}		
        //check available slot based on day of week
        $method = 'get' . date('D', strtotime($date));
        if ($slot->{$method}() == "0") {
            { return $this->ajaxerror=Mage::helper('ddate')->__('This time slot is not available in ').date('l', strtotime($date));}
        }       
        ...
        if (($slot->getSpecialday() == "1") && isset($special_date[$date])) {
            { return $this->ajaxerror=Mage::helper('ddate')->__('This date is special day');}
        }        
        ...
        if (isset($specifiedSpecial[$date])){
            { return $this->ajaxerror=Mage::helper('ddate')->__('This time slot is not available in this day'); }
        }        
        return 'ok';
    }
	public function update_commentAction() {
		$data=$this->getRequest()->getPost();		
		if(!empty($data['orderid'])){	
		$ddates_store = Mage::getModel('ddate/ddate_store')->getCollection()->addFieldToFilter('increment_id', $data['orderid'])->getFirstItem();
			if($ddates_store){
				$ddates_store->setDdateComment($data['mwcomment'])->save();
				echo $this->build_return_message('ok',Mage::helper('ddate')->__('Updated successfully!'));
			}else echo $this->build_return_message('error',Mage::helper('ddate')->__('Find no order with this id'));
